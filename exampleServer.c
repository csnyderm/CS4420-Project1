...
#define SERVER_PORT 8080                             /* arbitrar y, but client & server must agree */
...
{ int s, b, l, fd, sa, bytes, on = 1;
  ...
  struct sockaddr_in_channel;                       /* holds IP address */

  /* Build address structure to bind to socket. */
  memset(&channel, 0, sizeof(channel));              /* zero channel */
  channel.sin family = AF_INET;
  channel.sin addr.s_addr = htonl(INADDR_ANY);
  ...
  while (1) {
      ...
      while (1) {
           ...
           write(sa, buf, bytes);                   /* wr ite bytes to socket */
      }
      close(fd);                                     /* close file */
